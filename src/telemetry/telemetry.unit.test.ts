/**
 * @license
 * Copyright 2026 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */

import { expect } from 'chai';
import sinon, { SinonSpy, SinonFakeTimers } from 'sinon';
import type vscode from 'vscode';
import { Disposable } from 'vscode';
import { newVsCodeStub, VsCodeStub } from '../test/helpers/vscode';
import { ClearcutClient, ColabLogEventBase } from './client';
import { initializeTelemetry, telemetry } from '.';

const NOW = Date.now();
const SESSION_ID = 'sessionId';
const VERSION_COLAB = '0.1.0';
const VERSION_JUPYTER = '2025.0.0';
const VERSION_VSCODE = '1.109.0';

describe('Telemetry Module', () => {
  let disposeTelemetry: Disposable | undefined;
  let extensionContext: vscode.ExtensionContext;
  let fakeClock: SinonFakeTimers;
  let vs: VsCodeStub;

  beforeEach(() => {
    fakeClock = sinon.useFakeTimers({ now: NOW, toFake: [] });
    vs = newVsCodeStub();
    vs.extensions.getExtension.withArgs('ms-toolsai.jupyter').returns({
      isActive: true,
      packageJSON: {
        name: '',
        publisher: '',
        version: VERSION_JUPYTER,
      },
    } as vscode.Extension<unknown>);
    vs.env.sessionId = SESSION_ID;
    vs.version = VERSION_VSCODE;

    extensionContext = {
      extension: {
        packageJSON: { name: '', publisher: '', version: VERSION_COLAB },
      },
    } as vscode.ExtensionContext;
  });

  afterEach(() => {
    sinon.restore();
    disposeTelemetry?.dispose();
  });

  describe('lifecycle', () => {
    it('throws if doubly initialized', async () => {
      disposeTelemetry = await initializeTelemetry(
        extensionContext,
        vs.asVsCode(),
      );

      const secondInit = initializeTelemetry(extensionContext, vs.asVsCode());

      await expect(secondInit).to.eventually.be.rejectedWith(
        /already been initialized/,
      );
    });

    it('no-ops silently before being initialized', () => {
      expect(() => {
        telemetry.logActivation();
      }).not.to.throw();
    });

    it('disposes the client when disposed', async () => {
      const disposeSpy = sinon.spy(ClearcutClient.prototype, 'dispose');
      disposeTelemetry = await initializeTelemetry(
        extensionContext,
        vs.asVsCode(),
      );

      disposeTelemetry.dispose();

      sinon.assert.calledOnce(disposeSpy);
    });
  });

  describe('logs to Clearcut', () => {
    let baseLog: ColabLogEventBase;
    let logSpy: SinonSpy;

    beforeEach(async () => {
      logSpy = sinon.spy(ClearcutClient.prototype, 'log');
      baseLog = {
        extension_version: VERSION_COLAB,
        jupyter_extension_version: VERSION_JUPYTER,
        session_id: SESSION_ID,
        ui_kind: 'UI_KIND_DESKTOP',
        vscode_version: VERSION_VSCODE,
        timestamp: new Date(NOW).toISOString(),
      };
      disposeTelemetry = await initializeTelemetry(
        extensionContext,
        vs.asVsCode(),
      );
    });

    it('on activation', () => {
      telemetry.logActivation();

      sinon.assert.calledOnceWithExactly(logSpy, {
        ...baseLog,
        activation_event: {},
      });
    });

    it('on error', () => {
      const e = new Error('message');
      e.name = 'ErrorName';
      e.stack = 'stack';

      telemetry.logError(e);

      sinon.assert.calledOnceWithExactly(logSpy, {
        ...baseLog,
        error_event: { name: 'ErrorName', msg: 'message', stack: 'stack' },
      });
    });

    it('with the correct time', () => {
      const curTime = fakeClock.tick(100);

      telemetry.logActivation();

      sinon.assert.calledOnceWithExactly(logSpy, {
        ...baseLog,
        activation_event: {},
        timestamp: new Date(curTime).toISOString(),
      });
    });
  });
});
